...
'.$content.'
        ...
        protected function _drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
        {
            ...
            if (is_array($inTL) && count($inTL) >= 2) {
                if ($inTL[0] <= 0 || $inTL[1] <= 0) {
                    $inTL = null;
                }
            } else {
                $inTL = null; // handle jika $inTL bukan array atau tidak memiliki minimal dua elemen
            }
            
            if (is_array($inTR) && count($inTR) >= 2) {
                if ($inTR[0] <= 0 || $inTR[1] <= 0) {
                    $inTR = null;
                }
            } else {
                $inTR = null; // Handle jika $inTR bukan array atau tidak memiliki minimal dua elemen
            }
            
            if (is_array($inBR) && count($inBR) >= 2) {
                if ($inBR[0] <= 0 || $inBR[1] <= 0) {
                    $inBR = null;
                }
            } else {
                $inBR = null; // Handle jika $inBR bukan array atau tidak memiliki minimal dua elemen
            }
            
            if (is_array($inBL) && count($inBL) >= 2) {
                if ($inBL[0] <= 0 || $inBL[1] <= 0) {
                    $inBL = null;
                }
            } else {
                $inBL = null; // Handle jika $inBL bukan array atau tidak memiliki minimal dua elemen
            }
